...
export class CommandList implements IBaseCommand {
    ...
    private getFileInfo(filePath: string): IFileInfo {
    ...
    private execute(options: { all: boolean }): void {
        try {
            ...
            .map(file => this.getFileInfo(join(currentDir, file)))
            .sort((a, b) => {
            ...
            const output = fileInfos.map(file => {
                ...
                return `${name.padEnd(40)} ${
                    colors.yellow(this.formatSize(file.size).padEnd(10))} ${
